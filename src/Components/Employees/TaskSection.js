...
const StatCard = ({ value, label, tone = 'primary', index = 0 }) => (
  <div className={`stat-card tone-${tone} animate-fade-up`} style={{ animationDelay: `${index * 90}ms` }}>
...
const StatusPill = ({ status }) => {
  const map = { 'Pending': 'pending', 'In Progress': 'inprogress', 'Completed': 'completed' };
  const key = (status || '').toLowerCase();
  return <span key={key} className={`status-pill ${map[status] || 'pending'} animate-pulse-once`}>{status}</span>;
};
  ...
  return (
    <main className="main animate-page-in">
      ...
      <section className="stats-row">
        ...
        <StatCard value={inProgress} label="In Progress" tone="info" index={1} />
        <StatCard value={pending} label="Pending" tone="warning" index={2} />
        <StatCard value={completed} label="Completed" tone="success" index={3} />
      </section>
      ...
      <section className="panel animate-rise">
        ...
        <form onSubmit={handleAddTask} className="task-form premium">
          ...
          <div className="form-actions">
            <button type="submit" className="primary-btn tap">Add Task</button>
          </div>
        </form>
      </section>
      ...
      <section className="panel animate-rise" style={{ animationDelay: '80ms' }}>
        <div className="panel-header">
          ...
          <div className="toolbar">
            <button className="ghost-btn tap">Filters</button>
            <button className="ghost-btn tap">Export</button>
          </div>
        </div>
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((task, i) => (
                  <tr
                    key={task._id}
                    className={`row-animate ${task.status === 'Completed' ? 'completed-row' : ''}`}
        ...
        <footer className="table-footer">
          ...
          <div className="pager">
            ...
            <span className="page">1–6 of {Math.max(tasks.length, 6)}</span>
            <button className="ghost-btn tap">{'>'}</button>
          </div>
        </footer>
      </section>
