...
function EmployeeHome() {
  ...
  const isPunchInTimeAllowed = () => {
    ...
    return hour < 12 || (hour === 10 && minute <= 45);
  };
  ...
  // Always compare DATE STRINGS, not Date objects (avoids timezone bugs)
  const todayKey = new Date().toLocaleDateString("en-CA"); // "YYYY-MM-DD"

  let dayStatusClass = "";

  if (isoKey > todayKey) {
    dayStatusClass = "day-future";       // Future => blank (white)
  } else if (punchedIn) {
    dayStatusClass = "day-present";      // Present => green
  } else {
    dayStatusClass = "day-absent";       // Past but no punch => red
  }

  return (
    <div key={isoKey} className={`calendar-cell day-cell ${dayStatusClass}`}>
